...
const getUserChannelProfile = asyncHandler(async(req, res) => {
    ...
    const channel = await User.aggregate([
        ...
        },
        {
            $addFields: {
                subscribersCount: {
                    $size: "$subscribers" //'$' is used as above we used 'as'
                },
...
const getWatchHistory = asyncHandler(async(req, res) => {
    const user = await User.aggregate([
        {
            $match: {
                _id: new mongoose.Types.ObjectId(req.user._id) // we can't write _id:req.user?._id as it is moongoose aggreation pipeline
            }
        },
        {
            $lookup: {
